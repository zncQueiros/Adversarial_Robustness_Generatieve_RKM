...
def pkPCA(X):
    ...
    if X.size(0) > opt.h_dim:
        sigma = torch.sum(s[opt.h_dim:]) / (X.size(0) * (X.size(0)-opt.h_dim))
        noise = sigma*torch.eye(a.size(0)).to(opt.device)
        h, s, _ = torch.linalg.svd(a + noise, full_matrices=False)
...
U, h, s = final_compute(opt, net1, pkPCA)
...
plt.title('Training and validation loss for the training of the genRKM with pkPCA')
