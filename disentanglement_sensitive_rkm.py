...
def disentanglement_aware_regularization(model, x, gamma=2):
    ...
    for i in range(h.size(1)):
        h_perturbed = h.clone()
        h_perturbed[:, i] += gamma * torch.std(h[:, i])
...
while cost > 0.2 and t <= opt.max_epochs:  # run epochs until convergence
    ...
    for i, (datax, datay) in enumerate(xtrain):
        if i < math.ceil(opt.N / opt.mb_size):
            ...
            output = net1(datax)
            loss = rkm_loss(output, datax) + disentanglement_aware_regularization(model, datax)
