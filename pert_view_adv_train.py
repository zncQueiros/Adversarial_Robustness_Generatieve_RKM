...
parser.add_argument('--pert', type=float, default=30, help='amount of perturbation')
...
def rkm_loss2(output1, output2, X):
    h, s = kPCA2(output1, output2)
    U = torch.mm(torch.t(output1), h)
    V = torch.mm(torch.t(output2), h)

    x_tilde = net3(torch.mm(h, torch.t(U)))
    y_tilde = net3(torch.mm(h, torch.t(V)))
    ...
    recon_loss = torch.nn.MSELoss()
    f4 = recon_loss((x_tilde-y_tilde).view(-1, ipVec_dim), X.view(-1, ipVec_dim))  # reconstruction loss
...
net2 = Net1().float().to(opt.device)
net3 = Net3().float().to(opt.device)
net4 = Net3().float().to(opt.device)
    ...
    for i, (datax, datay) in enumerate(xtrain):
        if i < math.ceil(opt.N / opt.mb_size):
            ...
            if t > opt.delay and opt.adv:
                ...
                x_pert = x_adv - datax
                output_pert = net1(x_pert)
                loss = (loss + rkm_loss2(output1_adv, output_pert, datax))/2
                # h_adv, s_adv = kPCA(output1_adv)
                # U_adv = torch.mm(torch.t(output1_adv), h_adv)
                # delta_h = h - h_adv     # assumption, still need to map
                #
...
opt.shuffle = False
xt, _, _ = get_dataloader(args=opt)  # loading data without shuffle   # 3136x128 without linear layer
datax = xt.dataset.train_data[:opt.N, :, :, :].to(opt.device)
x_adv = adv_train(datax, model, adversary).to(opt.device)
output_adv = net1(x_adv)
x_pert = x_adv - datax
output_pert = net2(x_pert)
h, s = kPCA2(output_adv, output_pert)
U = torch.mm(torch.t(output_adv), h)
V = torch.mm(torch.t(output_pert), h)
